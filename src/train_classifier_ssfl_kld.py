...
def main():
    ...
    for i in range(cfg['num_experiments']):
        ...
        cfg['client_db_path'] = f"DB/semifl/{prefix}_{cfg['model_tag']}_client.db"
        cfg['server_db_path'] = f"DB/semifl/{prefix}_{cfg['model_tag']}_server.db"
...
def train_client(batchnorm_dataset, server_dataset_train, client_dataset, server, client, optimizer, metric, logger, epoch):
    ...
    for i in range(num_active_clients):
        ...
        cfg['participant_frequency'][selected_clients[i]] += 1
        client[selected_clients[i]].participant_frequency = cfg['participant_frequency'][selected_clients[i]]

    server.distribute(client, batchnorm_dataset)
    num_active_clients = len(selected_clients)
    start_time = time.time()
    lr = optimizer.param_groups[0]['lr']
    for i in range(num_active_clients):
        m = selected_clients[i]
        ...
        if i % int((num_active_clients * cfg['log_interval']) + 1) == 0:
            ...
            info = {'info': ['Model: {}'.format(cfg['model_tag']),
                             ...
                             'ID: {}({}/{})'.format(selected_clients[i], i + 1, num_active_clients),
